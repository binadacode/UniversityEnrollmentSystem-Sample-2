    ...
    public void modifyPersonInfo() {
        ...
        while (!found) {
            for (Person person : personList) {
                if (searchID.equalsIgnoreCase(person.getID())) {
                    ...
                    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
                    LocalDate newDob = LocalDate.parse(input.nextLine(), formatter);

                    person.setDob(newDob);
                    switch (person.getClass().getSimpleName().toLowerCase()){
                        case("student"):

                            System.out.println("Enter new course title: ");
                            ((Student) person).setCourseTitle(input.nextLine());
                            System.out.println("Enter no of modules enrolled: ");
                            ((Student) person).setModulesEnrolled(input.nextInt());
                            input.nextLine();
                            break;
                        case("lecturer"):

                            System.out.println("Enter new office number: ");
                            ((Lecturer) person).setOfficeNumber(input.nextInt());
                            input.nextLine();
                            break;
                        case("supportstaff"):
                            System.out.println("Enter new role: ");
                            ((SupportStaff) person).setRole(input.nextLine());
                            System.out.println("Enter new shift hours: ");
                            ((SupportStaff) person).setShiftHours(input.nextInt());
                            input.nextLine();
                            break;
                        default:
                            System.out.println("Error - person type not found");
                    }
                    found = true;
                } else{
